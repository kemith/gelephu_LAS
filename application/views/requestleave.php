...
<div class="row">
    <div class='col-sm-3'>
        ...
        <table class="table table-bordered table-hover table-striped leavetable">
        	<tr ><th>#</th><th>Leave Type</th><th>Start Date</th><th>End Date</th><th>Leave Count</th><th>Status</th><th>Remarks</th><th>Supervisor Remarks</th></tr>
        <?php $count=1; foreach($leavehistory->result() as $row): ?>
        	<tr><td><?php echo $count++;?></td><td><?php echo $row->leaveType;?></td><td><?php echo $row->startDate;?></td>
        		...
        		<td><?php if($row->status=="Pending"){echo '<div class="yellow center">'.$row->status."</div><a href='".base_url()."index.php/Leave/cancel/".$row->lid."' class='btn btn-default warning'>Cancel</a>";}
        		elseif($row->status=="Approved"){echo '<div class="green center">'.$row->status.'</div>';} else echo '<div class="red center">'.$row->status.'</div>'; ?></td>
        		<td><?php echo $row->remarks;?></td>
        		<td><?php echo $row->sRemarks;?></td></tr>
        	<?php endforeach; ?>
        </table>
...
function checkBalance(){
	...
	if((remarks!=null&&remarks!="")&&(leavetype!=null&&leavetype!="")&&(startDate!=null&&startDate!="")&&(endDate!=null&&endDate!="")){
		if(leavedays>0){
				if(leavetype=='4'){
...
function submitLeave(){
	...
	var endDate = $('#datetimepickerend').val();
	var remarks = $('#remarks').val();
	$.post('<?php echo base_url();?>index.php/Leave/submitleave/',
	{
		startdate:startDate,
		enddate:endDate,
		leavedays:leavedays,
